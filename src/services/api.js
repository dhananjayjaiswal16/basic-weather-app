...
const API_KEY = "5842ba6bda2744be11a5a62a0c99e102";
...
export const getFormattedWeather = async (searchParams) => {
  try {
    const formattedCurrentWeather = await getWeather('weather', searchParams).then(formatWeather);

    const { lat, lon } = formattedCurrentWeather;

    const formattedForecast = await getWeather("onecall", {
